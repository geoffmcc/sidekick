...
| B2 | Legacy handler extraction | **done** (#241–#245) | Extract the legacy handlers in the documented dependency order. Each a reviewable slice. | Zero production handlers in `tools-legacy.js`; the canonical registry owns 108 built-in descriptors, including Compute, while active module, capability-pack, and generated tools join the same registry. The remaining legacy file provides compatibility policy/approval/audit machinery and derived projections; no family imports it at initialization. |
...
| B6 | Artifact custody convergence | **done** | Register worker-uploaded artifacts in the kernel; surface (not swallow) mirror failures; reconcile pre-existing orphans; enforce project/principal-scoped reads. | `src/compute/artifact-custody.js` is the one custody path; `finalizeArtifact` registers with the kernel under the compute artifact id (idempotent via primary key). Custody failures are recorded on the row, published as `compute.artifact_custody_failed`, and logged. Orphan reconciliation is dry-run by default. Current artifact projections enforce authenticated access plus project/principal/resource filtering, covered by the artifact-resource-scope and identity-resource-authority suites. No schema change. See `docs/artifact-custody.md`. |
| B7 | Connector integration | **keystone and fast-follow landed in current source** | GitHub is registered as a managed connector; the `github` tool routes endpoint + credential (secret_ref -> secret store) through the connector authority; read-only connector inspection, bounded health, events, and governed lifecycle routes are available. | Broader provider coverage remains configuration-dependent. See `docs/connectors.md`. |
