...
sudo install -d -m 0750 -o root -g sidekick /etc/sidekick
sudo tee /etc/sidekick/sidekick.env >/dev/null <<'EOF'
SIDEKICK_BIND_HOST=YOUR_SERVER_PRIVATE_IP
SIDEKICK_DASHBOARD_BIND_HOST=YOUR_SERVER_PRIVATE_IP
EOF
sudo chown root:sidekick /etc/sidekick/sidekick.env
sudo chmod 0640 /etc/sidekick/sidekick.env
...
Fresh installations bind MCP and Dashboard to loopback by default. For a
dedicated server, set both `SIDEKICK_BIND_HOST` and
`SIDEKICK_DASHBOARD_BIND_HOST` in `/etc/sidekick/sidekick.env` to the server's
private LAN or VPN address. Do not use `localhost` for remote clients and do
not use `0.0.0.0` unless public wildcard exposure is intentional and
`SIDEKICK_ALLOW_PUBLIC_BIND=1` is explicitly configured. After changing the
file, run `sudo systemctl daemon-reload` and restart the affected services.
