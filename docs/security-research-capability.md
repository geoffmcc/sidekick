...
The generic connector contract owns registration, configuration, lifecycle, opaque secret references, health and event metadata. The first domain connector must be defined against the working `security-research` surface and tested independently of the unavailable Workbench. Do not claim Workbench-backed integration until its endpoint, lifecycle and result contract are verified.
