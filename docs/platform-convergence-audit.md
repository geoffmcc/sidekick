...
| Tool ownership | **converged (B1/B2 done)** | On current `main`, the canonical registry owns 108 built-in descriptors, including Compute; active module, capability-pack, and generated tools join that same registry. `src/tools-legacy.js` contains compatibility policy/approval/audit machinery and derived projections, not production handlers or canonical descriptor ownership. | Compatibility exports remain only where consumers require them; they are not alternate execution authorities. |
