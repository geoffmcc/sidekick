...
| Artifacts — access auth | **missing** | `GET /api/artifacts` has no auth/project scoping; `checkCapability` never invoked for artifacts. | Add auth + scoping. |
| Connectors — lifecycle | **production-complete (records)** | Registration/config/state-machine/redaction-on-write + REST endpoints. | — |
| Connectors — integrations | **foundation-only (governs no traffic)** | Only provider is a declared stub; no connector-type→implementation dispatch. All real integrations (GitHub/Slack/Ollama/OpenAI) bypass the framework. `secret:` refs regex-validated but **unresolvable** (no bridge to secret store). No project binding; two read endpoints unauthenticated; no UI. | First real provider through the framework; secret-ref resolver; auth. |
