...
| `capability` | Services | Inspect and manage capability packs; install or enable activates executable module code inside the Sidekick process | `{ action: "string (list|available|show|inspect|validate|install|configure|enable|disable|health|doctor|upgrade|uninstall - default list)", name: "string (pack name)", path: "string (server-local package path)", config: "object (optional)", enable: "boolean (optional)", allow_same_version: "boolean (optional)", allow_downgrade: "boolean (optional)", remove_knowledge: "boolean (optional)", remove_module_data: "boolean (optional)" }` |
