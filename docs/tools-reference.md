...
This guide describes the descriptor registry and compatibility projections
exported through `src/tools/index.js`. The catalog is dynamic: core, module,
capability-pack, and approved generated tools share one registry, and installed
packs can add tools at runtime. The live `tools` table, `tools` manifest, and
`capability` catalog are authoritative for current names, schemas, counts,
categories, risk, policy, and enabled/deprecated state.
...
Tool definitions exposed by the dashboard API include policy metadata. This is a reference to the live governed catalog, not a frozen count: core descriptors, modules, capability packs, and approved generated tools can change the inventory at runtime. Use `tools({ action: "overview" })` or `GET /api/tools` for the authoritative current set.
...
|---|---|---|
| Critical | `bash`, `write`, `db_restore`, `runbook`, `ops`, `mission`, `sandbox`, `evolve`, `capability`, `memory_maintenance` | Gate in shared or public deployments. Allow only for trusted operators. |
| High | `process`, `service`, `module`, `workflow`, `project_registry`, `workspace`, `browser`, `cron`, `delay`, `watch`, `github`, `teach`, `secret`, `db_migrate`, `queue`, `orchestrate`, `health`, `netdiag`, `baseline`, `tunnel`, `wireguard`, `nginx`, `compute_providers` | Block in `restricted` mode unless the workflow needs them. |
| Medium | `notify`, `read`, `archive`, `git`, `web_fetch`, `llm`, `context`, `session`, `handoff`, `memory`, `memory_import`, `memory_manage`, `sync_import`, `snapshot`, `retry`, `fresheyes`, `batch`, `tail`, `find`, `status`, `extract`, `changelog`, `timeline`, `circuit`, `depend`, `black_box`, `predict`, `db_query`, `db_backup`, `db_export`, `redis`, `ocr`, `media`, `transcribe`, `analytics`, `download`, `compute`, `compute_nodes`, `compute_jobs`, `compute_route`, `connector` | Generally useful, but can expose data or trigger external effects. |
...
The table below is a practical reference, not a frozen or exhaustive schema
contract. Before an unfamiliar or consequential call, use `tools action="get"`
or `tools action="policy"` with the canonical unprefixed name. Use
`tools action="overview"` or `GET /api/tools` to enumerate the current
deployment. Capability packs and approved generated tools may extend this list.
...
| `llm` | Core | Ask the LLM through Compute placement | `{ prompt: "string", system: "string (optional)", temperature: "number (optional)", async: "boolean (optional)", timeout_ms: "number (optional)", max_tokens: "number (optional)", output_budget: "string (optional, normal|complex|large)", context_limit: "number (optional)" }` |
...
| `handoff` | Context & Learning | First-class versioned handoff storage with structured packets, evidence, checkpoints, claims, readiness, resume, lifecycle, and audit history | `{ action: "string (create|update|get|list|versions|restore|compare|inspect|validate|verify|start_here|quality|preflight|simulate_resume|refresh_evidence|checkpoint|readiness|events|transition|claim|renew_claim|begin_resume|release|reprocess|archive|unarchive|purge_version)", id: "string (optional)", project: "string (optional)", title: "string (optional)", content: "string (optional)", packet: "object (optional structured resume packet)", source: "string (optional)", task_id: "string (optional)", include_archived: "boolean (optional)", include_completed: "boolean (optional; list completed lifecycle records)", limit: "number (optional)", version: "number (optional)", expected_version: "number (optional)", reason: "string (optional audit reason)", working_directory: "string (optional)", owner: "string (optional)", lease_seconds: "number (optional)", lifecycle_state: "string (optional)" }` |
| `memory` | Context & Learning | Typed memory operations for remember, query, explain, correct, forget, pin, expire, conflicts, and health | `{ action: "string (remember|query|explain|list|get|confirm|correct|forget|pin|expire|conflicts|health)", id: "string (optional)", project: "string (optional)", type: "string (optional)", memory_class: "string (optional)", content: "string (for remember)", summary: "string (optional)", scope_type: "string (optional)", scope_id: "string (optional)", source: "string (optional)", evidence: "string (optional)", confidence: "number (optional)", tags: "string|array (optional)", query: "string (optional)", limit: "number (optional)", correct_to: "string (optional)", fresh_eyes: "boolean (optional)", historical: "boolean (optional)" }` |
...
| `secret` | Security | Encrypted credential management with AES-256-GCM (requires the configured file-backed secret-key reference) | `{ action: "string (store|get|delete|list|rotate)", key: "string (secret name)", value: "string (optional, for store)", generate: "string (optional, length for rotate, e.g. '32')" }` |
...
| `predict` | Meta | Evidence-backed prediction and decision-support engine. Analyzes correlated tool history, incidents, and workflows within an explicit scope to identify likely next actions, failure risks, missing prerequisites, incident recurrence, and workflow opportunities. Every persisted prediction passes an evidence and confidence admission gate. | `{ action: "string (analyze|list|get|feedback|outcome|dismiss|explain|status|suggest|migrate|purge_preview|purge|diagnose)", id: "string (optional, prediction id)", type: "string (optional, filter by prediction type)", scope: "string (optional, project|session|task|global - required for analyze; use global to deliberately analyze every project)", project: "string (optional)", session_id: "string (optional)", task_id: "string (optional)", feedback: "string (optional, useful|not_useful|incorrect|already_known|acted_on|dismissed)", outcome: "string (optional, confirmed|did_not_occur|action_succeeded|action_failed|expired|superseded|unresolved)", status: "string (optional, filter by status)", confidence: "string (optional, filter by confidence)", limit: "number (optional, default 20, max 100)", maxAge: "string (optional, analysis window - default 7d)", confirm: "boolean (optional, required true to execute purge)", retention_days: "number (optional, override retention for purge_preview/purge)" }` |
...
| `transcribe` | Media | Transcribe audio/video using a bounded synchronous or asynchronous Whisper-compatible backend | `{ action: "string (submit|status|cancel)", path: "string (optional, required for submit)", model: "string (optional)", language: "string (optional)", async: "boolean (optional)", job_id: "string (optional)", backend: "string (optional, auto|faster-whisper|whisper.cpp|whisper)", device: "string (optional, auto|cpu|cuda|metal|vulkan)" }` |
...
| `embed` | Context & Learning | Generate text embeddings through Compute placement | `{ text: "string", model: "string (optional)", device: "string (optional, GPU|NPU)" }` |
...
| `module` | Services | Inspect and operate platform module lifecycle state through the shared policy and approval path | `{ action: "string (list|get|health|status|check|recover|enable|disable - default list)", name: "string (module name)" }` |
| `capability` | Services | Inspect the shared capability catalog, prove maturity, and manage capability packs; install or enable activates executable module code inside the Sidekick process | `{ action: "string (list|catalog|available|show|inspect|validate|install|configure|enable|disable|health|maturity|prove|record_verification|doctor|upgrade|uninstall - default list)", name: "string (pack name)", path: "string (server-local package path)", config: "object (optional)", source: "string (catalog policy source)", query: "string (catalog filter)", kind: "string (pack|module|tool|workflow)", owner: "string (catalog owner)", state: "string (catalog state)", available: "boolean (catalog availability)", offset: "number", limit: "number", enable: "boolean (optional)", allow_same_version: "boolean (optional)", allow_downgrade: "boolean (optional)", remove_knowledge: "boolean (optional)", remove_module_data: "boolean (optional)" }` |
| `workflow` | Services | List, inspect, run, and resume registered workflows; steps execute through the governed dispatcher with durable state and approval continuation | `{ action: "string (list|show|run|resume - default list)", name: "string (workflow name)", inputs: "object (optional)", project: "string (optional)", run_id: "string (optional)", owner: "string (optional)", include_evidence: "boolean (optional)" }` |
| `connector` | Services | Inspect registered connectors, run bounded health checks, and read lifecycle events without exposing credential values | `{ action: "string (list|get|events|health - default list)", connector_id: "string (optional)", type: "string (optional)", state: "string (optional)", limit: "number (optional)" }` |
| `project_registry` | Storage | Canonical project registry for listing, inspecting, registering, archiving, and backfilling project data sources | `{ action: "string (list|get|register|archive|sources|backfill)", project: "string (optional/required by action)", state: "string (optional, active|archived)", limit: "number (optional)", display_name: "string (optional)", description: "string (optional)", reason: "string (optional)", confirm: "boolean (required for confirmed backfill)", dry_run: "boolean (optional, default true)" }` |
| `workspace` | Storage | Manage project workspaces and encrypted workspace secrets; secret values are never returned and backfill is dry-run by default | `{ action: "string (list|get|create|update|set_secret|delete_secret|backfill_secrets)", workspace_id: "string (optional/required by action)", project: "string (optional)", name: "string (optional)", config: "object (optional)", environment: "string (optional)", resource_limits: "object (optional)", metadata: "object (optional)", secret_name: "string (optional)", secret_value: "string (optional)", state: "string (optional)", limit: "number (optional)", confirm: "boolean (optional)", dry_run: "boolean (optional, default true)" }` |
| `browser` | Networking | Operate an isolated governed Chromium session with egress policy, bounded interaction, secret references, screenshots, downloads, and artifact custody | `{ action: "string (open|close|list|status|navigate|back|forward|reload|snapshot|extract|click|fill|secret_fill|clear|select|check|press|hover|focus|scroll|wait|screenshot|upload|downloads|assert|pages|switch_page|close_page|sequence)", session: "string (required except open/list/status)", url: "string (optional)", target: "string|object (optional)", secret_ref: "string (optional)", allowed_hosts: "array (optional)", allow_private_network: "boolean (optional)", steps: "array (optional for sequence)", timeout_ms: "number (optional)" }` |
...
| `compute_providers` | Compute | Manage production compute providers such as Ollama, OpenAI-compatible, vLLM, llama.cpp, and MLX | `{ action: "string (list|get|create|update|delete|health|health_all)", provider_id: "string (optional)", name: "string (optional)", type: "string (optional, ollama|openai|vllm|llamacpp|mlx)", base_url: "string (optional)", api_key: "string (optional, never expose or paste live credentials)", priority: "number (optional)", enabled: "boolean (optional)", trust_level: "string (update only)", mode: "string (optional, direct|worker)", tls_policy: "string (optional, require|prefer|off)", cost_policy: "string (optional)", data_classifications: "array (update only)" }` |
...
| `compute_jobs` | Compute | Manage allowlisted compute jobs, statistics, cancellation, retry/recovery, and artifacts | `{ action: "string (list|get|create|cancel|retry|recover|stats|artifacts|reconcile_artifact_custody)", job_id: "string (optional)", job_type: "string (chat|generate|embeddings|text_embedding for create)", capability: "string (optional)", request_payload: "object (optional)", capability_requirements: "object (optional)", data_classification: "string (public|internal|private)", model: "string (optional)", prompt: "string (optional)", provider: "string (optional)", output_budget: "string (normal|complex|large)", source_job_id: "string (optional)", max_retries: "number (optional)", timeout_ms: "number (optional)", idempotency_key: "string (optional)", status: "string (optional)", project: "string (optional)", confirm: "boolean (optional, required for custody reconciliation)" }` |
...
| `memory_maintenance` | Context & Learning | Governed project-scoped bulk memory hygiene with deterministic previews, transactional apply, status, cancellation, and resumable runs | `{ action: "string (preview|apply|status|cancel|resume)", project: "string", run_id: "string (optional)", max_candidates: "number (optional, 1-500)", handoff_retention_days: "number (optional)", purge_handoff_versions: "boolean (optional, irreversible)", purge_approved: "boolean (explicit approval required for purge)" }` |
...
Ask the LLM through Compute placement. Provider and model selection are routing
decisions; callers do not select credentials or provider endpoints.

Arguments: `{ prompt: "string", system: "string (optional)", temperature: "number (optional)", async: "boolean (optional)", timeout_ms: "number (optional)", max_tokens: "number (optional)", output_budget: "string (optional, normal|complex|large)", context_limit: "number (optional)" }`
...
Evidence-backed prediction and decision-support engine. Analyzes correlated tool history, incidents, and workflows within an explicit scope to identify likely next actions, failure risks, missing prerequisites, incident recurrence, and workflow opportunities. Every persisted prediction passes an evidence and confidence admission gate.

`analyze` requires an explicit scope: pass `project`, `session_id` or `task_id`, or request `scope: "global"` deliberately to analyze every project. `purge` requires `confirm: true` and should be preceded by `purge_preview`. See [Predict](predict.md) for detectors, thresholds, lifecycle, and retention policy.

Arguments: `{ action: "string (analyze|list|get|feedback|outcome|dismiss|explain|status|suggest|migrate|purge_preview|purge|diagnose)", id: "string (optional, prediction id)", type: "string (optional, filter by prediction type)", scope: "string (optional, project|session|task|global - required for analyze; use global to deliberately analyze every project)", project: "string (optional)", session_id: "string (optional)", task_id: "string (optional)", feedback: "string (optional, useful|not_useful|incorrect|already_known|acted_on|dismissed)", outcome: "string (optional, confirmed|did_not_occur|action_succeeded|action_failed|expired|superseded|unresolved)", status: "string (optional, filter by status)", confidence: "string (optional, filter by confidence)", limit: "number (optional, default 20, max 100)", maxAge: "string (optional, analysis window - default 7d)", confirm: "boolean (optional, required true to execute purge)", retention_days: "number (optional, override retention for purge_preview/purge)" }`
...
Encrypted credential management with AES-256-GCM (requires the configured file-backed secret-key reference)
