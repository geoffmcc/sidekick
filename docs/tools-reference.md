...
| High | `process`, `service`, `module`, `workflow`, `project_registry`, `workspace`, `browser`, `cron`, `delay`, `watch`, `github`, `teach`, `secret`, `db_migrate`, `queue`, `orchestrate`, `health`, `netdiag`, `baseline`, `tunnel`, `wireguard`, `nginx` | Block in `restricted` mode unless the workflow needs them. |
...
| `handoff` | Context & Learning | First-class **versioned** handoff storage and ingestion with idempotent, redacted, evidence-linked memory extraction. Every content update preserves the prior version in append-only history; `update` requires an existing handoff and supports optimistic concurrency via `expected_version`; `versions` lists history, `get` accepts a `version` selector, and `restore` appends a historical version as the new latest (history is never rewritten). `purge_version` is the deliberate, audited remediation path for a historical version containing accidentally pasted sensitive data: it requires an explicit `reason`, never touches the current version, and records the removal in the memory audit trail. | `{ action: "string (create|update|get|list|versions|restore|compare|inspect|reprocess|archive|unarchive|purge_version)", id: "string (optional)", project: "string (optional)", title: "string (optional)", content: "string (for create/update)", source: "string (optional)", task_id: "string (optional)", include_archived: "boolean (optional)", limit: "number (optional)", version: "number (get: historical version; restore/purge_version: target version)", expected_version: "number (update: fail if current version differs)", reason: "string (purge_version: required audit reason)" }` |
...
| `module` | Services | Inspect and operate platform module lifecycle state through the shared policy and approval path | `{ action: "string (list|get|enable|disable - default list)", name: "string (module name for get/enable/disable)" }` |
| `capability` | Services | Inspect the shared capability catalog and manage capability packs; install or enable activates executable module code inside the Sidekick process | `{ action: "string (list|catalog|available|show|inspect|validate|install|configure|enable|disable|health|doctor|upgrade|uninstall - default list)", name: "string (pack name)", path: "string (server-local package path)", config: "object (optional)", source: "string (catalog policy source)", query: "string (catalog filter)", kind: "string (pack|module|tool|workflow)", owner: "string (catalog owner)", state: "string (catalog state)", available: "boolean (catalog availability)", offset: "number", limit: "number", enable: "boolean (optional)", allow_same_version: "boolean (optional)", allow_downgrade: "boolean (optional)", remove_knowledge: "boolean (optional)", remove_module_data: "boolean (optional)" }` |
