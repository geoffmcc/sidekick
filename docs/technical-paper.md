...
3. Query KV/context state for project-specific continuity.
...
The `meta` table stores `schema_version`. Migration files live in `migrations/` and use numeric prefixes such as `001_initial_schema.sql` and `002_tool_registry.sql`.
...
The `context` document is Sidekick's structured continuity store. It contains explicit `sidekick_context` entries for projects, decisions, problems, patterns, and sessions, plus automatic `memories` generated by `src/memory.js`. Automatic memory records bounded, redacted summaries of memory-worthy tool calls and completed Agent Bridge tasks. It is enabled by default, disabled with `SIDEKICK_AUTO_MEMORY=0`, and capped by `SIDEKICK_AUTO_MEMORY_MAX`.
