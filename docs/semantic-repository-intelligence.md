...
Parsing uses pinned Tree-sitter grammars for TypeScript, JavaScript, Ruby, Java, Go, Perl, and Rust. Adapters expose parser metadata and normalize symbols, imports, exports, and statically observed calls into the versioned IR. Parser errors remain visible as bounded warnings; the index never executes repository code. Native parser packages are installed as ordinary locked runtime dependencies and are not a requirement for the target language runtimes themselves.
...
`dev_change_summary` and semantic queries expose bounded added, removed, and changed file/symbol information. Relevant Agent requests can contribute a small `repository_semantic` context source through the existing governed tool dispatcher and Brain/context ranking pipeline. Repository-derived content is explicitly untrusted data and is never treated as Sidekick instructions.
