...
function createTaskRunner({ taskEmitters, taskCancels, emit, runAgent, redactSensitive, onTaskCreated = null }) {
  return function beginTaskRun(res, { goal, goalSpec = null, parentContext = null, profile = "standard", workspaceRef = null, authorityEnvelope = null, handoffId = null, taskId: requestedTaskId = null, resume = false }) {
    const taskId = requestedTaskId || crypto.randomUUID().slice(0, 8);
    let creation = null;
    try { if (onTaskCreated) creation = onTaskCreated({ taskId, goal, goalSpec, parentContext, profile, workspaceRef, authorityEnvelope, handoffId, resume }); } catch (error) {
