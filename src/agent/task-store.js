...
function insertTask(task) {
  ...
  const tx = db.transaction(() => {
    ...
    if (task.handoff_id) db.prepare("UPDATE agent_tasks SET handoff_id = ? WHERE task_id = ?").run(String(task.handoff_id).slice(0, 180), task.task_id);
    appendEventInternal(db, task.task_id, "task.created", task.actor_id, { state: task.state, profile: task.profile, ...(task.handoff_id ? { handoff_id: String(task.handoff_id).slice(0, 180) } : {}) });
  });
  tx();
