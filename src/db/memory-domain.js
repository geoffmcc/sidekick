function createMemoryDomain({ db, hasMemoriesTable, normalizeMemoryRow, nowIso }) {
  ...
  function getMemoryById(id, { includeDisabled = true } = {}) { if (!hasMemoriesTable() || !id) return null; return normalizeMemoryRow(db.prepare(`SELECT * FROM memories WHERE id = ? ${includeDisabled ? "" : "AND enabled = 1 AND COALESCE(current, 1) = 1"}`).get(id)); }
  ...
  function enableMemory(id) { if (!hasMemoriesTable()) return false; const ts = nowIso(); return db.prepare("UPDATE memories SET enabled = 1, current = 1, updated_at = ?, last_seen_at = ? WHERE id = ? AND state NOT IN ('deleted', 'expired', 'superseded')").run(ts, ts, id).changes > 0; }
