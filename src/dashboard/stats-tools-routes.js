...
function registerStatsToolsRoutes({ app, dbStore, getToolDefsForSource, errorResponse }) {
  app.get("/api/stats", (req, res) => {
    try {
      ...
      return errorResponse(req, res, error, { status: 500, code: "service_unavailable", component: "stats" });
    }
  });
  ...
  app.get("/api/tools", (req, res) => {
    try {
      ...
      return res.json({ tools, total: tools.length, limit, query: query || null });
    } catch (error) {
      return errorResponse(req, res, error, { status: 500, code: "service_unavailable", component: "tools" });
    }
  });
}
