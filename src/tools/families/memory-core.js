...
async function sidekick_memory({ action, id, project, type, memory_class, content, summary, scope_type, scope_id, source, evidence, confidence, tags, query, limit, reason, correct_to, fresh_eyes, historical }) {
  ...
  if (action === "query" || action === "list") {
    const memories = fresh_eyes ? [] : dbStore.searchMemories({ query, project, type: type || "all", limit: limit || 20, includeDisabled: historical === true }).filter(m => historical === true || (m.current !== false && m.state !== "expired" && m.state !== "deleted"));
    return jsonText({ ok: true, count: memories.length, memories, brief: query ? buildScopedMemoryBrief(query, project, { type: type || "all", limit: limit || 10 }) : null, fresh_eyes: fresh_eyes === true });
  }
  ...
  if (action === "backfill") {
    const keys = Object.entries(dbStore.getAllKV()).map(([key, entry]) => ({ key, ...(entry || {}) })).filter(entry => (!project || entry.project === project) && /handoff|resume|plan|next[_ -]?step/i.test(`${entry.key} ${entry.category || ""}`)).slice(0, limit || 25);
