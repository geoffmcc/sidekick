...
function collectReliabilityMetrics({ db = dbDefault, limit = 10000 } = {}) {
  ...
  const rows = db.getDb().prepare("SELECT state,created_at,updated_at,completed_at,usage_json,result_json,verification_json FROM agent_tasks ORDER BY updated_at DESC LIMIT ?").all(bounded);
