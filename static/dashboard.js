    ...
    list.innerHTML = projections.map(projection => {
      ...
      return '<div class="card mission-panel mission-panel-wide">' +
        ...
        '<div class="mission-muted">Next: ' + esc(start.next_step || 'No next step recorded') + '</div>' +
         ...
         '<details><summary>Receiver details</summary><pre class="agent-log handoff-details">' + esc(JSON.stringify({ current_state: start.current_state || null, next_safe_action: start.next_step || null, completed_steps: projection.completed_steps || [], remaining_steps: projection.remaining_steps || [], remaining_plan: projection.plan || null, continuation: projection.continuation || null, decisions: start.decisions || [], blockers, open_questions: questions, risks: start.risks || [], acceptance_criteria: projection.acceptance_criteria || [], artifacts: projection.artifacts || [], relationships: projection.relationships || [], provenance: projection.provenance || null, evidence: evidence.items || [], claim: projection.claim || null, reasons: readiness.reasons || quality.issues || [] }, null, 2)) + '</pre></details>' +
        '</div>';
