...
function testMatches(name) {
  ...
  return testFiles.filter(file => {
    ...
    return candidate.includes(normalized.toLowerCase()) || candidate.includes("pack") || candidate.includes("certification");
  }).map(relative).sort();
}
...
function inventoryPack(packDir) {
  ...
  return {
    ...
    current_tests: testMatches(manifest.name),
    maturity_gaps: gaps,
    overlaps: [],
