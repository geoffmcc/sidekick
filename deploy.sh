...
verify_git_deploy_source() {
  local helper="$PROJECT_DIR/scripts/git-deploy.js"
  if [ ! -f "$helper" ]; then
    echo -e "\033[31mERROR: Git deployment helper not found at $helper\033[0m"
    exit 1
  fi
  if ! git -C "$PROJECT_DIR" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    echo -e "\033[31mERROR: Git deploy mode must run from a Git working tree\033[0m"
    exit 1
  fi

  local branch origin_url status head origin_head
  branch=$(git -C "$PROJECT_DIR" branch --show-current)
  origin_url=$(git -C "$PROJECT_DIR" remote get-url origin)
  if [ "$branch" != "main" ]; then
    echo -e "\033[31mERROR: Git deploy mode requires local main; current branch is ${branch:-detached}\033[0m"
    exit 1
  fi
  if [ "$origin_url" != "$EXPECTED_REPO_URL" ] || [[ "$origin_url" == *"@github.com"* ]]; then
    echo -e "\033[31mERROR: Git deploy mode requires clean origin $EXPECTED_REPO_URL\033[0m"
    exit 1
  fi

  git -C "$PROJECT_DIR" fetch --prune origin main >/dev/null
  status=$(git -C "$PROJECT_DIR" status --porcelain)
  if [ -n "$status" ]; then
    echo -e "\033[31mERROR: Git deploy mode requires a clean local working tree\033[0m"
    exit 1
  fi
  head=$(git -C "$PROJECT_DIR" rev-parse HEAD)
  origin_head=$(git -C "$PROJECT_DIR" rev-parse origin/main)
  if [ "$head" != "$origin_head" ]; then
    echo -e "\033[31mERROR: Git deploy mode requires local main to match origin/main\033[0m"
    exit 1
  fi
}
...
if [ "$SCP_MODE" = true ]; then
  ...
  if [ -f "$PROJECT_DIR/.env" ]; then
    ...
    if [ "$remote_env_exists" = "YES" ]; then
      ...
      run_remote "chmod 600 $REMOTE_DIR/.env" >/dev/null
      changed+=(".env")
    fi
  else
  ...
  verify_git_deploy_source
  if ! copy_to_vps "$PROJECT_DIR/scripts/git-deploy.js" "$GIT_DEPLOY_HELPER" >/dev/null; then
    echo -e "\033[31mERROR: Failed to upload Git deployment helper\033[0m"
    exit 1
  fi
  run_remote "chmod 700 $GIT_DEPLOY_HELPER" >/dev/null
  remote_has_git=$(run_remote "test -d $REMOTE_DIR/.git && echo YES || echo NO" 2>/dev/null) || true
  git_mode="deploy"
  if [[ "$remote_has_git" != *"YES"* ]]; then
    git_mode="convert"
    echo -e "  \033[33mConverting existing deployment to read-only Git working tree...\033[0m"
    if [ -f "$PROJECT_DIR/.env" ]; then
      remote_env_exists=$(run_remote "test -f $REMOTE_DIR/.env && echo YES || echo NO")
      if [ "$remote_env_exists" != "YES" ]; then
        echo -e "\033[32mSyncing .env before first Git conversion...\033[0m"
        if ! copy_to_vps "$PROJECT_DIR/.env" "$REMOTE_DIR/.env" >/dev/null; then
          echo -e "\033[31mERROR: Failed to copy .env\033[0m"
          exit 1
        fi
  ...
  if ! run_remote "node $GIT_DEPLOY_HELPER $git_mode 2>&1"; then
    echo -e "\033[31mERROR: Git deployment helper failed\033[0m"
    exit 1
  fi
  changed+=("git")
  git_mode_used=true
...
if [ "$git_mode_used" = true ]; then
  echo ""
  echo -e "\033[36m--- Restarting MCP ---\033[0m"
