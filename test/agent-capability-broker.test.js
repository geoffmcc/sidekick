...
const { discoverCapabilities, selectRelevantContextProvider, hasRelevantContextProvider, buildAgentCapabilityMetadata, resolveContextProviderArgs } = require("../src/agent/capability-broker");
...
const catalog = [
  ...
  { name: "semantic_repo", description: "Understand unfamiliar repositories, architecture, symbols, dependencies, authentication, network and process boundaries", category: "Development", source: "module:developer-tools", contextProvider: { tool: "semantic_repo", action: "query", source: "repository_semantic" } },
];
...
assert.ok(hasRelevantContextProvider("Analyze this repository", catalog, {}), "generic repository analysis discovers a declarative context provider");
assert.ok(selectRelevantContextProvider("Analyze this repository", catalog, {}), "positive provider relevance selects the provider");
assert.ok(!hasRelevantContextProvider("Tell me a joke", catalog, {}), "irrelevant requests do not force repository inspection");
assert.strictEqual(classifyEvidenceRequirement("Analyze this repository", { repositoryCapabilityDiscovered: true }).reason, "capability_context_provider");
...
const crossToolContextDescriptor = normalizeDescriptor({ name: "safe_context", description: "Safe context", schema: z.object({}), risk: "low", contextProvider: { tool: "bash", action: "run", source: "unsafe" }, handler: () => null });
assert.strictEqual(crossToolContextDescriptor.contextProvider, null, "low-risk descriptors cannot nominate unrelated automatic providers");
