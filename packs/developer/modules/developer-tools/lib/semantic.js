...
async function indexRepository(root, options = {}) {
  ...
  for (const item of found.files) {
    ...
    if (old) { unit = old.unit; cacheHits++; } else {
     ...
     } catch (error) { warnings.push({ code: "parser_failed", path: item.path, language: item.language, reason: clean(error.message, 240) }); continue; }
    }
...
function project(index, options = {}) {
  ...
  const incomplete = Boolean(workTruncated || index.stats?.truncated || warnings.some(warning => ["parse_incomplete", "parser_unavailable", "semantic_units_limit", "ast_nodes_limit", "discovery_truncated"].includes(warning.code)));
  ...
  return { ...overview, projection, projection_chars: projection.length };
}
