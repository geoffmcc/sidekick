...
async function contract(services, args) {
  ...
  const assertions = args.assertions.slice(0, services.config?.max_assertions || 20);
...
async function matrix(services, args) {
  ...
  return result({ ok: results.length > 0 && results.every(item => item.ok), tool: "api_contract_matrix", network_scope: args.network_scope, results, bounded: { targets: results.length, max_targets: args.targets.length } });
}
...
]; }, healthCheck({ config }) { return { ok: Number.isInteger(config?.max_assertions || 20) && (config?.max_assertions || 20) > 0, details: { max_assertions: config?.max_assertions || 20, dependencies: ["web_check", "web_extract"], network_policy: "named network scope required for every target" } }; } };
