...
- `research_run action=provision` composes the Proxmox pack's `proxmox_provision` to create a disposable guest from an operator-supplied spec, and records the resource identity as a custody artifact linked to the run (provenance). `research_run action=cleanup` composes `proxmox_guest` for an authorized graceful shutdown and reports deletion as **pending/manual** — the Proxmox pack exposes no VM delete by design, and cleanup never reintroduces a destructive path. `proxmox_provision` and `proxmox_guest` are added to the module's permission allowlist (the tools it now genuinely dispatches).
- **Which** lab (endpoint, profile, `token_ref`, clone spec) is supplied entirely by runtime configuration — the Proxmox pack's profiles/secret store and the security-research pack's `environments` config — and is never committed. The public repo carries only the generic composition and synthetic examples.
- Fail-closed: an absent Proxmox pack returns `dependency_missing`, a provider refusal (e.g. `lifecycle_disabled`) returns `policy_denied`, and a non-`proxmox` environment returns `unsupported_operation`. Verified offline with a stub provider (`test/security-research-pack.test.js` SR.17–SR.19).

### Security Research capability pack

A first-party bundled pack (`packs/security-research`) that makes Sidekick a governed, reproducible, auditable framework for **authorized** security research. It is a consumer and orchestrator of existing capabilities, not a new monolith: it exposes the platform kernel's already-existing research record layer (campaigns, hypotheses, test runs, findings, reports; migrations 032–035) as a governed tool/workflow surface, and adds only the pieces the kernel deliberately does not own.

- **Ten tools** (`research_status`, `research_project`, `research_hypothesis`, `research_scope`, `research_run`, `research_probe`, `research_evidence`, `research_compare`, `research_validate`, `research_report`) and **two workflows** (`security-research/source-regression` read-only, `security-research/version-regression-check` mutating). Command probes compose the governed `bash` tool (local host, opt-in) and HTTP probes compose `web_fetch` — the module's only two allowlisted dispatch targets; the `source-regression` workflow composes `git`; a `proxmox` environment composes the Proxmox pack for disposable-lab provisioning (see the entry above). The pack has no special privilege and cannot bypass provider policy.
- **Hard public/private boundary.** All target-specific research lives in an external, configurable workspace (`workspace` config or `SIDEKICK_RESEARCH_WORKSPACE`). The workspace is canonicalized through `realpath` and refused, fail-closed, if it resolves to, contains, or is contained by the Sidekick repository, data directory, or managed pack store — or if it is dangerously shallow. Evidence bytes live only in the workspace; the kernel stores references, SHA-256 hashes and original/derivative lineage via artifact custody, and `research_evidence inspect` returns metadata only.
- **Deterministic by design.** Hashing, comparison, scope evaluation, state tracking and evidence association are decided by code and work with no model at all. Probes are typed, bounded, scope- and SSRF-gated, and audited; command probes on the Sidekick host are off by default; a completed run and a confirmed finding require evidence, enforced by the kernel.
- **Leakage protection.** A scoped static check (`test/static-code-quality.test.js`) scans the pack surface (`packs/security-research`, its tests, its docs) for developer-machine paths and secrets, and the pack unit suite self-scans the pack tree.
- **Tests:** `test/security-research-unit.test.js` (22 boundary/gating/comparison/self-scan checks) and `test/security-research-pack.test.js` (16 integration checks driving install → campaign/hypothesis/scope/run → probes composing bash → evidence integrity/redaction → comparison/validation → report material → scope enforcement → both workflows, fully offline against synthetic local fixtures).
- Core change: registered `security-research` as a known platform event source (`src/platform/event-vocabulary.js`), matching the `proxmox-pack` precedent.
